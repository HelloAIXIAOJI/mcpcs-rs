    ...
    pub async fn connect(&self, config: &McpServerConfig) -> Result<RunningService<RoleClient>> {
        match config {
            McpServerConfig::Legacy { command, args, .. } => {
                ...
                let transport = TokioChildProcess::spawn(cmd)?;
                let client = ().serve(transport).await?;
                Ok(client)
            }
            McpServerConfig::ChildProcess { transport } => {
                let mut cmd = Command::new(&transport.command);
                cmd.args(&transport.args);

                if let Some(env) = &transport.env {
            ...
            McpServerConfig::Sse { transport } => {
                use rmcp::transport::sse_client::SseClientConfig;

                // 🚀 OpenAI方法：在reqwest客户端层面设置headers
                ...
                let config = SseClientConfig::new(&transport.url);
